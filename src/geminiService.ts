...
export const generateDepthMap = async (imageB64: string, apiKey?: string): Promise<string | undefined> => {
  const ai = getAiClient(apiKey);
  const { mimeType, data } = parseB64(imageB64);
  ...
  const response = await ai.models.generateContent({
    ...
    contents: {
      parts: [
        { inlineData: { data, mimeType } },
        { text: "Generate a high-contrast grayscale depth map for this image. Foreground objects (the product) MUST be bright white, and the background MUST be deep black. Ensure the silhouettes are sharp and the object is clearly visible. DO NOT return a black image." }
      ]
    },
    config: {
      imageConfig: {
...
export const generateThreeDImage = async (params: {
  originalImageB64: string,
  warpedDepthMapB64: string,
  prompt: string,
  model: string,
  aspectRatio: string,
  imageSize?: string,
  ...
  const original = parseB64(params.originalImageB64);
  const warped = parseB64(params.warpedDepthMapB64);
  ...
  const response = await ai.models.generateContent({
    ...
    contents: {
      parts: [
        ...
        { inlineData: { data: warped.data, mimeType: warped.mimeType } },
        { text: `Task: Redraw the product from the original image using the perspective and spatial structure provided by the warped depth map. 
          Prompt: ${params.prompt}. 
          Requirements: 
          1. Maintain the exact product identity, textures, and branding from the original image.
          2. Follow the 3D perspective and orientation shown in the warped depth map.
          3. Ensure professional studio lighting and high-end product photography quality.` }
      ]
    },
    config: {
      imageConfig: {
...
export const generate3DModel = async (imageB64: string, model: string, apiKey?: string): Promise<string> => {
  const ai = getAiClient(apiKey);
  const { mimeType, data } = parseB64(imageB64);
  
  try {
    // We ask Gemini to extract the product and return it on a pure white background.
    // This makes the 3D preview look much better as a "model".
    const response = await ai.models.generateContent({
      model: 'gemini-2.5-flash-image',
      contents: {
        parts: [
          { inlineData: { data, mimeType } },
          { text: "Extract the main product from this image and place it on a PURE WHITE background. Do not include any shadows or props. Only the product itself. The output MUST be the product image on white." }
        ]
      },
      config: {
        imageConfig: {
    ...
    console.error("Error generating 3D preview texture:", error);
  }
