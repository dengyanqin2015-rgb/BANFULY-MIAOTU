...
export const MaskEditNode = ({ data, selected }: NodeProps<Node<MaskEditNodeData>>) => {
  ...
  return <>
    <div className="flex w-[320px] flex-col gap-1.5 text-white">
      ...
      <div className={`relative w-full overflow-visible rounded-lg bg-[#1a1a1a] shadow-2xl ${selected ? 'ring-2 ring-cyan-400 ring-offset-2 ring-offset-[#0a0a0a]' : 'border border-[#333]'}`}>
        <Handle type="target" position={Position.Left} className="!z-20 !h-3 !w-3 !border-2 !border-[#0a0a0a] !bg-cyan-400" />
        <button onClick={() => setOpen(true)} className="group relative block w-full overflow-hidden rounded-lg bg-[#0a0a0a]">
        ...
        <Handle type="source" position={Position.Right} className="!z-20 !h-3 !w-3 !border-2 !border-[#0a0a0a] !bg-cyan-400" />
      </div>
    ...
    {open && createPortal(<ImageTextEditor imageUrl={data.imageUrl} title="遮罩定点修改" onClose={() => setOpen(false)} onConfirm={(imageUrl) => { data.onApply?.(imageUrl); setOpen(false); }} onBackgroundTask={runInBackground} backgroundActive={working} initialDraft={draft} onDraftChange={preserveDraft}/>, document.body)}
  </>;
};
