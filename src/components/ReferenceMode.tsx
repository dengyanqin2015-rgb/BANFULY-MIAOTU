...
import { ImageDeconstruction, MODEL_COSTS } from '../types';
import { Upload, Zap, Download, Share2, Box } from 'lucide-react';

interface DeconstructionField {
  key: string;
  label: string;
}
...
interface ReferenceModeProps {
  ...
  setSingleRefImage: (url: string | null) => void;
  isDeconstructing: boolean;
  deconstructionResult: ImageDeconstruction | null;
  runDeconstruction: (base64: string) => void;
  ...
  DECONSTRUCTION_FIELDS: DeconstructionField[];
  editablePrompt: string;
  setEditablePrompt: (prompt: string) => void;
  useRefImage: boolean;
  setUseRefImage: (use: boolean) => void;
  refStrength: number;
  setRefStrength: (strength: number) => void;
  productKeywords: string;
  setProductKeywords: (keywords: string) => void;
}
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
      ...
      <div className="lg:col-span-5 space-y-8">
        <div className="apple-card p-8 bg-white border-black/10 shadow-xl">
          <div className="section-label mb-6 text-[#0071e3]">Step 01 / Visual Deconstruction</div>
          <h2 className="text-2xl font-black mb-6 tracking-tight">上传参考图 <span className="text-[#86868b]">解构审美基因</span></h2>
          ...
          >
            {singleRefImage ? (
              ...
              <div className="text-center p-8">
                ...
                <p className="text-lg font-bold mb-2">点击上传参考图</p>
                <p className="text-[#86868b] text-sm">支持 JPG, PNG (建议高质量摄影图)</p>
              </div>
            )}
          </div>
            ...
            onChange={(e) => {
              ...
              if (file) {
                ...
                reader.onload = (event) => {
                  ...
                  setSingleRefImage(b64);
                  runDeconstruction(b64);
                };
          ...
          {isDeconstructing && (
            <div className="mt-8 p-6 bg-[#f5f5f7] rounded-2xl border border-black/5 animate-pulse">
              <div className="flex items-center gap-4 mb-4">
                ...
                <p className="text-sm font-black text-black">AI 正在深度解构画面基因...</p>
              </div>
          ...
          {deconstructionResult && (
            <div className="mt-8 space-y-4 animate-in fade-in slide-in-from-top-4 duration-700">
              <div className="grid grid-cols-2 gap-3">
                {DECONSTRUCTION_FIELDS.slice(0, 4).map((f) => (
                  <div key={f.key} className="p-4 bg-[#fbfbfd] rounded-2xl border border-black/5">
                    <p className="text-[10px] font-bold text-[#86868b] uppercase tracking-widest mb-1">{f.label.split(' ')[0]}</p>
                    <p className="text-[12px] font-black text-black truncate">{(deconstructionResult as unknown as Record<string, string>)[f.key] || '自动提取中'}</p>
                  </div>
                ))}
              </div>
              <div className="p-4 bg-[#0071e3]/5 rounded-2xl border border-[#0071e3]/10">
                <p className="text-[10px] font-bold text-[#0071e3] uppercase tracking-widest mb-1">画面情绪基调</p>
                <p className="text-[13px] font-black text-black">{deconstructionResult.mood_tone}</p>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="lg:col-span-7 space-y-8">
        <div className="apple-card p-8 bg-white border-black/10 shadow-xl">
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div>
                <div className="flex items-center justify-between mb-4">
                  <label className="text-sm font-bold text-[#1d1d1f]">风格参考强度</label>
                  <span className="text-[12px] font-black text-[#0071e3] bg-[#0071e3]/5 px-2 py-1 rounded-lg">{(refStrength * 100).toFixed(0)}%</span>
                </div>
                <input 
                  type="range" 
                  min="0" 
              ...
              <div>
                <label className="block text-sm font-bold mb-4 text-[#1d1d1f]">产品核心关键词</label>
                <input 
                  type="text"
                  placeholder="例如：白色陶瓷花瓶, 极简主义..."
                  value={productKeywords}
                  onChange={(e) => setProductKeywords(e.target.value)}
                  className="w-full px-5 py-3.5 rounded-xl bg-[#fbfbfd] border-2 border-black/5 outline-none focus:border-[#0071e3]/30 transition-all text-sm font-medium"
                />
              </div>
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                <label className="text-sm font-bold text-[#1d1d1f]">AI 提示词微调</label>
                <button 
                  onClick={() => setUseRefImage(!useRefImage)}
                  className={`text-[10px] font-black px-3 py-1 rounded-full transition-all ${useRefImage ? 'bg-[#0071e3] text-white' : 'bg-[#f5f5f7] text-[#86868b]'}`}
                >
                  {useRefImage ? '启用参考图' : '仅使用提示词'}
                </button>
              </div>
              <textarea 
                rows={4}
                ...
                placeholder="在此微调 AI 生成提示词..."
                className="w-full px-6 py-5 rounded-3xl bg-[#fbfbfd] border-2 border-black/5 outline-none focus:border-[#0071e3]/30 transition-all text-sm font-medium leading-relaxed resize-none"
              />
            </div>
            ...
            <div className="p-6 bg-[#f5f5f7] rounded-3xl border border-black/5 flex items-center justify-between">
              ...
              <button
                ...
                className={`px-10 py-4 rounded-2xl font-black text-lg transition-all duration-500 shadow-xl ${isGeneratingSingle || !singleProductImage || !deconstructionResult ? 'bg-[#e8e8ed] text-[#86868b] cursor-not-allowed' : 'bg-black text-white hover:bg-[#1d1d1f] hover:shadow-2xl hover:-translate-y-1 active:scale-95'}`}
              >
          ...
          <div className="apple-card p-8 bg-white border-black/10 shadow-xl animate-in fade-in slide-in-from-bottom-8 duration-1000">
            ...
            <div className="rounded-[32px] overflow-hidden border border-black/5 shadow-2xl group relative">
              ...
              <div className="absolute top-6 right-6 flex gap-3 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                <button 
                  onClick={() => downloadImage(singleGeneratedImage, 'reference-view-result.png')}
